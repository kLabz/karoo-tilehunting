...
class StatshuntersTilesProvider(
    ...
    suspend fun requestTiles(shareCode: String): Flow<ActivitiesWithLines> = flow {
        var page = 1
        do {
            Log.d(SquadratingExtension.TAG, "Requesting page $page...")

            val newActivities = requestTilePage(shareCode, page)
            val newLines = try {
                requestLinePage(shareCode, page)
            } catch(e: HttpDownloadError){
                Log.e(SquadratingExtension.TAG, "Failed to download lines", e)
    ...
    private suspend fun requestTilePage(shareCode: String, page: Int = 1): ActivityListResponse {
        val client = HttpClient(Android) {
            ...
            defaultRequest {
                userAgent(SquadratingExtension.TAG)
            }
        }
        ...
        try {
            ...
            Log.d(SquadratingExtension.TAG, "Http request to ${url}...")

            return client.get(url).body<ActivityListResponse>().also {
                Log.d(SquadratingExtension.TAG, "Parsed activity response with ${it.activities.size} activities")
            }
        } catch (e: ClientRequestException) {
            throw HttpDownloadError(e.response.status.value, e.message)
        } catch (e: Exception) {
            Log.e(SquadratingExtension.TAG, "Failed to fetch tiles", e)
            throw e
        } finally {
            client.close()
        }
    ...
    private suspend fun requestLinePage(shareCode: String, page: Int = 1): LinesListResponse {
        ...
        try {
            ...
            Log.d(SquadratingExtension.TAG, "Http lines request to ${url}...")
            ...
            Log.e(SquadratingExtension.TAG, "Failed to fetch lines", e)
            throw e
        } finally {
            client.close()
        }
