...
class RecentlyExploredNewSquadratsDataType(
    private val karooSystem: KarooSystemService,
    private val applicationContext: Context
) : DataTypeImpl("squadrating", "explored_new_squadrats_trip") {
    override fun startStream(emitter: Emitter<StreamState>) {
        val job = CoroutineScope(Dispatchers.IO).launch {
            applicationContext.exploredSquadratsDataStore.data.collect { exploredSquadrats ->
                val count = exploredSquadrats.recentlyExploredNewSquadratsCount
                emitter.onNext(
